...
async function detectAuthenticationRequired(page, statusCode, finalUrl, verbose = false) {
  ...
  try {
    ...
    const hasAuthIndicator = authIndicators.some(indicator =>
      titleLower.includes(indicator) || bodyText.includes(indicator)
    );
    ...
    if (verbose && (hasAuthIndicator || hasAuthUrl)) {
      console.error(`Authentication indicators detected: title="${title}", hasAuthUrl=${hasAuthUrl}`);
    }
...
Options:
  ...
  --chromium-path <path> Path to Chromium executable (default: /Applications/Chromium.app/Contents/MacOS/Chromium)
